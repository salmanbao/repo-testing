...
export const dropScrt = async (transaction: DropTransaction) => {
...
export const testSendScrt = async (receiver: string, amount: number) => {
    const vaultWallet = new Wallet(VAULT_SCRT_KEYPAIR);
    const userAddress = receiver;
    const scrtAmount = Math.ceil(amount * 1_000_000).toString();
