...
​
- We **don't** create PR's with **console.log**'s in the code
...
const modifiedFiles = danger.git.modified_files.filter(
    (path) => path.endsWith('js') || path.endsWith('tsx') || path.endsWith('ts'),
);
const modifiedHtmlFiles = danger.git.modified_files.filter((path) => path.endsWith('html'));

// Check for console.log statements
modifiedFiles.forEach((file) => {
    // exclude dangerfile from check
    if (file === 'dangerfile.ts') return;
...
// check whether any style attribute was left in html files
modifiedHtmlFiles.forEach((file) => {
    const regExp = /style="(\w.*?)"/g;
    const content = fs.readFileSync(file).toString();
    let match;
    while ((match = regExp.exec(content)) != null) {
        fail(`\`${match[1]}\` - style attribute was found in file (\`${file}\`). Please use (s)css files for styling.`);
    }
});
